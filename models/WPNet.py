...
"""
    author      : liang
    datatime    : 2017-09-10 23:58
    description : whole_part_net : 
"""
...
# endregion
    ...
    def composite_function(self, x, out_features, kernel_size=3):
        ...
        with tf.variable_scope("composite_function"):
            ...
            # convolution
            x = self.conv2d(
                x, out_features=out_features, kernel_size=kernel_size)
    ...
    def add_internal_layer(self,name, _input, out_features):
        """Perform H_l composite function for the layer and after concatenate
        input with output from composite function.
        """
        with tf.variable_scope(name):
            bottleneck_out = self.bottleneck(_input, out_features=out_features)
            b = self.composite_function(
                bottleneck_out, out_features=out_features, kernel_size=3)
        return b

    def add_block(self, x, group_num,growth_rate=1, repeat=1):
        for j in range(repeat):
            print(j+1,'*' * 30)
            orig_growth = x.shape[-1].value // group_num
            group_growth =int((growth_rate+1)*orig_growth)
            n=0
            for i in range(group_num):
                with tf.variable_scope("plus_{}_{}".format(j,i)):
                    b = self.add_internal_layer('b_{}'.format(i) , x , group_growth)
                    if n+group_growth>x.shape[-1]:
                        b += tf.concat(axis = 3 ,values = (x[: , : , : , n :],x[:,:,:,:group_growth-orig_growth]))
                    else:
                        b += x[:,:,:,n:n+group_growth]
                    x = tf.concat(axis = 3 ,values = (x[:,:,:,:n] ,b,x[:,:,:,n+orig_growth:]))
                    n += group_growth
                    print(i+1,'-' * 5 , orig_growth , '-' * 5 , n , '-' * 5 , x.shape[-1].value)
        print('='*30)
        return x
    ...
    def transition_layer(self, x):
        ...
        channels = x.get_shape()[-1] * self.compress_rate
        output = self.composite_function(
            x, out_features=channels, kernel_size=1)
        # run average pooling
