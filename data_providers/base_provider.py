...
class ImagesDataSet(DataSet):
    ...
    def _measure_mean_and_std(self):
        ...
        self._means = means
        self._stds = stds
        print("--data--mean----------------------------------- = ",means,"--data--std--- = ",stds)
    ...
    def normalize_images(self, images, normalization_type):
        ...
        if normalization_type == 'divide_255':
            images = images / 255
            print('_______data__divide_255_______')
        elif normalization_type == 'divide_256':
            images = images / 256
            print('_______data__divide_256_______')
            ...
            for i in range(images.shape[-1]):
                images[:, :, :, i] = ((images[:, :, :, i] - self.images_means[i]) /
                                       self.images_stds[i])
                print('_______data__by_chanels_______')
        else:
            images = self.sub_mean_stds(images)
            print('_______data__sub_mean_______')
        return images

    def sub_mean_stds(self,dataset) :
